...
codeunit 82564 "ADLSE Util"
{
    ...
    var
        ...
        MinutePart: Text;
        SecondPart: Text;
        DateTxt: Label '%1, %2 %3 %4 %5:%6:%7 GMT', Comment = '%1: Weekday, %2: Day, %3: Month, %4: Year, %5: Hour, %6: Minute, %7 Seconds';
    begin
    ...
    var
        OrigTableName: Text;
        TableNameTok: Label '%1-%2', Comment = '%1: TableName, %2: TableID';
    begin
        OrigTableName := GetTableName(TableID);
        TableName := GetDataLakeCompliantName(OrigTableName);
        exit(StrSubstNo(TableNameTok, TableName, TableID));
    end;

    procedure GetDataLakeCompliantFieldName(FieldName: Text; FieldID: Integer): Text
    var
        FieldNameTok: Label '%1-%2', Comment = '%1: FieldName, %2: FieldID';
    begin
        exit(StrSubstNo(FieldNameTok, GetDataLakeCompliantName(FieldName), FieldID));
    end;
    ...
    var
        ValueTok: Label '"%1"', Comment = '%1: Value';
    begin
        exit(StrSubstNo(ValueTok, Val));
    end;
    ...
    var
        ...
        StartIdx: Integer;
        PeriodIdx: Integer;
        SecondsTxt: Label ':%1Z', Comment = '%1: Seconds';
        WholeSecondsTxt: Label ':%1.%2Z', Comment = '%1: Whole Seconds, %2: Milliseconds';
    begin
        ...
        Result := Result.Replace(StrSubstNo(SecondsTxt, SecondsText), StrSubstNo(WholeSecondsTxt, WholeSecondsText, MillisecondsText));
    end;
    ...
    begin
        ...
        AppliedTraits := GetCDMAppliedTraits();
    end;
    ...
    local procedure GetCDMAppliedTraits() AppliedTraits: JsonArray
    begin
        Clear(AppliedTraits);
        // case Type of
        //     FieldType::DateTime:
    ...
    procedure CreateCsvPayload(RecRef: RecordRef; FieldIdList: List of [Integer]; AddHeaders: Boolean) RecordPayload: Text
    var
        ...
        FieldTextValue: Text;
        Payload: TextBuilder;
        PayloadTxt: Label ',%1', Comment = '%1: Field Text';
    begin
        FieldsAdded := 0;
        ...
        if AddHeaders then begin
            foreach FieldID in FieldIdList do begin
                ...
                if FieldsAdded = 0 then
                    ...
                    Payload.Append(StrSubstNo(PayloadTxt, FieldTextValue));
                FieldsAdded += 1;
            end;
            if IsTablePerCompany(RecRef.Number) then
                Payload.Append(StrSubstNo(PayloadTxt, ADLSECDMUtil.GetCompanyFieldName()));
            Payload.AppendLine();
        end;
        ...
        if IsTablePerCompany(RecRef.Number) then
            Payload.Append(StrSubstNo(PayloadTxt, ConvertStringToText(CompanyName())));
        Payload.AppendLine();
