...
codeunit 82563 "ADLSE Http"
{
    ...
    var
        ...
        ContentTypePlainTxt: Label 'text/plain; charset=utf-8', Locked = true;
        UnsupportedMethodErr: Label 'Unsupported method: %1', Comment = '%1: HttpMethod';
    ...
    var
        ...
        AccessToken: Text;
        AuthError: Text;
        AuthTok: Label 'Bearer %1', Comment = '%1: Access Token';
    begin
        ...
        Headers := Client.DefaultRequestHeaders();
        Headers.Add('Authorization', StrSubstNo(AuthTok, AccessToken));
    ...
    var
        ...
        ResponseBody: Text;
        Json: JsonObject;
        UriTok: Label 'https://login.microsoftonline.com/%1/oauth2/token', Comment = '%1: TenantID';
        ResourceTok: Label 'resource=%1&scope=%2&client_id=%3&client_secret=%4&client_info=1&grant_type=client_credentials', Comment = '%1: StorageURL, %2: StorageScope, %3: ClientID, %4: ClientSecret';
    begin
        Uri := StrSubstNo(UriTok, Credentials.GetTenantID());
