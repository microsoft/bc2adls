...
codeunit 82561 "ADLSE Execute"
{
        ...
        if Rec.FindSet(false) then begin
            ...
            repeat
                if ADLSECommunication.TryCollectAndSendRecord(Rec, TimeStampField.Value(), FlushedTimeStamp) then
                    ...
                    ErrMsg := GetLastErrorText() + GetLastErrorCallStack();
                    Error(ErrMsg);
                end;
            until Rec.Next() = 0;
    ...
    begin
        ...
        if ADLSEDeletedRecord.FindSet(false) then begin
            ...
            RecRef.Open(ADLSEDeletedRecord."Table ID");
            repeat
                ADLSEUtil.CreateFakeRecordForDeletedAction(ADLSEDeletedRecord, RecRef);
                if ADLSECommunication.TryCollectAndSendRecord(RecRef, ADLSEDeletedRecord."Entry No.", FlushedTimeStamp) then
