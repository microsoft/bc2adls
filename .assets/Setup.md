...
- **Allow simultaneous exports** The flag to enable the tool to export data from multiple companies at the same time. You should enable this only after the export schema is finalized- in other words, when you know exactly which tables, and which fields in those tables to export. We recommend that the json files are manually checked in the outbound container before enabling this flag. Changes to the export schema are not allowed as long as this flag is checked.
